        ...
        with st.expander(f'{name}的配置'):
            ...
            with col_level:
                st.number_input(f'音擎精炼等级', min_value=1, max_value=5, 
                              value=saved_config[name]['weapon_level'] if name in saved_config else 1, 
            ...
            # 主词条选择
                ...
                st.selectbox(f'四号位主词条', main_stat4_options, 
                                     index=main_stat4_options.index(saved_config[name]['drive4']) if name in saved_config else 0, 
                                     key=f'{name}_main_stat4')
                st.selectbox(f'五号位主词条', main_stat5_options, 
                                     index=main_stat5_options.index(saved_config[name]['drive5']) if name in saved_config else 0, 
                                     key=f'{name}_main_stat5')
                st.selectbox(f'六号位主词条', main_stat6_options, 
                                     index=main_stat6_options.index(saved_config[name]['drive6']) if name in saved_config else 0, 
            ...
            with col1:
                st.number_input(f'攻击力%', min_value=0, max_value=sc_max_value, value=saved_config[name]['scATK_percent'] if name in saved_config else 0, key=f'{name}_scATK_percent')
                st.number_input(f'攻击力', min_value=0, max_value=sc_max_value, value=saved_config[name]['scATK'] if name in saved_config else 0, key=f'{name}_scATK')
            with col2:
                st.number_input(f'生命值%', min_value=0, max_value=sc_max_value, value=saved_config[name]['scHP_percent'] if name in saved_config else 0, key=f'{name}_scHP_percent')
                st.number_input(f'生命值', min_value=0, max_value=sc_max_value, value=saved_config[name]['scHP'] if name in saved_config else 0, key=f'{name}_scHP')
            with col3:
                st.number_input(f'防御力%', min_value=0, max_value=sc_max_value, value=saved_config[name]['scDEF_percent'] if name in saved_config else 0, key=f'{name}_scDEF_percent')
                st.number_input(f'防御力', min_value=0, max_value=sc_max_value, value=saved_config[name]['scDEF'] if name in saved_config else 0, key=f'{name}_scDEF')
            with col4:
                st.number_input(f'暴击率', min_value=0, max_value=sc_max_value, value=saved_config[name]['scCRIT'] if name in saved_config else 0, key=f'{name}_scCRIT')
                st.number_input(f'暴击伤害', min_value=0, max_value=sc_max_value, value=saved_config[name]['scCRIT_DMG'] if name in saved_config else 0, key=f'{name}_scCRIT_DMG')
            with col5:
                st.number_input(f'异常精通', min_value=0, max_value=sc_max_value, value=saved_config[name]['scAnomalyProficiency'] if name in saved_config else 0, key=f'{name}_scAnomalyProficiency')
                st.number_input(f'穿透值', min_value=0, max_value=sc_max_value, value=saved_config[name]['scPEN'] if name in saved_config else 0, key=f'{name}_scPEN')
            
            # 暴击配平算法开关
            st.checkbox(f'使用暴击配平算法', value=saved_config[name]['crit_balancing'] if name in saved_config else False, key=f'{name}_crit_balancing')
