...
def buff_exist_judge(charname_box, judge_list_set, weapon_dict, cinema_dict):
    ...
    for names in charname_box:
        ...
        addition_skill_active_judge_info[names]["required_condition"] = (
            CHARACTER_FILE.loc[names, "组队被动条件"]
        )
    ...
    select_buff_dict = preprocess_dataframes(
        charname_box,
        EXIST_FILE,
        JUDGE_FILE,
        weapon_dict,
        addition_skill_active_judge_info,
        total_judge_condition_list,
        cinema_dict,
    )
    """
    ...
    for buff_name, buff_info_tuple in select_buff_dict.items():
        ...
        if buff_from in charname_box:
            ...
            for name in selected_characters:
                initiate_buff(
                    buff_info_tuple, buff_name, exist_buff_dict, name, buff_from
                )
...
def processor_equipment_buff(
    adding_code, buff_info_tuple, buff_name, exist_buff_dict, name_order_dict, sub_list
):
    ...
    for name0 in selected_characters:
        initiate_buff(
            buff_info_tuple, buff_name, exist_buff_dict, name0, equipment_carrier
        )
    ...
    for buff_name, row_data in exist_file.iterrows():
        ...
        if row_data["is_weapon"]:
            for charname in weapon_dict:
                if (
                    buff_from == weapon_dict[charname][0]
                    and row_data["refinement"] == weapon_dict[charname][1]
                ):
            ...
            if buff_from in weapon_dict:
                if row_data["is_additional_ability"]:
                    ...
                    """
                    condition_list_after_trans = addition_skill_info_trans(
                        addition_skill_active_judge_info, buff_from
                    )
                    ...
                    for other_key in addition_skill_active_judge_info:
                        if other_key != buff_from:
                            partner_condition_list.extend(
                                addition_skill_active_judge_info[other_key][
                                    "config_info"
                                ]
                            )
                    ...
                    for conditions in condition_list_after_trans:
                        if conditions in partner_condition_list:
                            select_buffs(
                                buff_name, judge_file, row_data, selected_buffs
                            )
                            break
                else:
...
if __name__ == "__main__":
    ...
    Judge_list_set = [
        [name_box[0], "玉壶青冰", "震星迪斯科", "摇摆爵士"],
        [name_box[1], "好斗的阿炮", "静听嘉音", "如影相随"],
        [name_box[2], "牺牲洁纯", "如影相随", "啄木鸟电音"],
    ]
    char_0 = {
        "name": name_box[0],
        "weapon": Judge_list_set[0][1],
        "weapon_level": 1,
        "equip_set4": Judge_list_set[0][2],
        "equip_set2_a": Judge_list_set[0][3],
        "drive4": "暴击率",
        "drive5": "电属性伤害",
        "drive6": "冲击力%",
        "scATK_percent": 10,
    ...
    char_1 = {
        "name": name_box[1],
        "weapon": Judge_list_set[1][1],
        "weapon_level": 5,
        "equip_set4": Judge_list_set[1][2],
        "equip_set2_a": Judge_list_set[1][3],
        "drive4": "暴击率%",
        "drive5": "穿透率",
        "drive6": "能量自动回复%",
        "scATK_percent": 10,
    ...
    char_2 = {
        "name": name_box[2],
        "weapon": Judge_list_set[2][1],
        "weapon_level": 1,
        "equip_set4": Judge_list_set[2][2],
        "equip_set2_a": Judge_list_set[2][3],
        "drive4": "暴击率",
        "drive5": "攻击力%",
        "drive6": "攻击力%",
        "scATK_percent": 10,
    ...
    weapon_dict = {
        name_box[0]: [char_0["weapon"], char_0["weapon_level"]],
        name_box[1]: [char_1["weapon"], char_1["weapon_level"]],
        name_box[2]: [char_2["weapon"], char_2["weapon_level"]],
    }
    cinema_dict = {
        name_box[0]: char_0["cinema"],
        name_box[1]: char_1["cinema"],
        name_box[2]: char_2["cinema"],
    }

    exist_buff_dict = buff_exist_judge(
        name_box, Judge_list_set, weapon_dict, cinema_dict
    )
    for names, buff_dict in exist_buff_dict.items():
        print(names)
        for buff_name, buff in buff_dict.items():
