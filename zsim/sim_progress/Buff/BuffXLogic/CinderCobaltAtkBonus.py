...
class CinderCobaltAtkBonus(Buff.BuffLogic):
    ...
    def check_record_module(self):
        if self.equipper is None:
            self.equipper = JudgeTools.find_equipper("「灰烬」-钴蓝", sim_instance=self.buff_instance.sim_instance)
    ...
    def special_judge_logic(self, **kwargs):
        ...
        if self.record.listener is None:
            self.record.listener = self.buff_instance.sim_instance.listener_manager.get_listener(listener_owner=self.record.char, listener_id=self.buff_instance.ft.listener_id)
