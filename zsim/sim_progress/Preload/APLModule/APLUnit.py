...
class SimpleUnitForForceAdd(APLUnit):
    ...
    def check_all_sub_units(self, found_char_dict, game_state, sim_instnace: "Simulator", **kwargs):
        result_box = []
        tick = kwargs.get("tick", None)
        if not self.sub_conditions_unit_list:
            return True, result_box
        for sub_units in self.sub_conditions_unit_list:
            ...
            result = sub_units.check_myself(found_char_dict, game_state, tick=tick, sim_instance=sim_instnace)
            result_box.append(result)
            if not result:
                return False, result_box
        else:
