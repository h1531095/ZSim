...
class QuickAssistSystem:
    ...
    def update(self, tick: int, skill_node, all_name_order_box: dict[str, list[str]]):
        ...
        if skill_node.skill.trigger_buff_level == 7:
            if not self.quick_assist_manager_group[skill_node.char_name].quick_assist_available:
                raise ValueError(f'在{skill_node.char_name}的快速支援没有亮起的情况下，打出了快速支援！')
    ...
    @staticmethod
    def spawn_event_group(tick_now: int, skill_node, active_manager: QuickAssistManager):
...
class QuickAssistEvent:
    """快速支援事件"""
    def __init__(self, update_tick: int, updated_by, operation: bool, manager: QuickAssistManager, answer: bool = False):
        ...
        if self.operation:
            ...
            if self.exit_mode:
                ...
                self.execute_tick = update_tick + self.updated_by.skill.aid_lag_ticks + self.manager.max_duration

    def execute_update(self, tick_now: int, answer: bool = False):
        """事件的自执行方法"""
        if tick_now != self.execute_tick and self.operation:
            raise ValueError(f'{self.manager.char.NAME}的本次快速支援的激活更新理应在{self.execute_tick}tick执行，但实际执行于{tick_now}tick')
        if self.operation:
