...
class ListenerManger:
    ...
    def listener_factory(self, listener_owner: "Character | Enemy | None", initiate_signal: str = None, sim_instance: "Simulator" = None):
        """初始化监听器的工厂函数"""
        ...
        if listener_owner is None:
            raise ValueError(f"调用监听器工厂函数时，listener_onwner参数不能为空！")
    ...
    def get_listener(self, listener_owner: "Character | Enemy | None", listener_id: str) -> BaseListener | None:
        """获取指定监听器"""
        owner_type = type(listener_owner).__bases__[0].__name__
        if owner_type == "Character":
