...
class YixuanAnomalyListener(BaseListener):
    ...
    def listening_event(self, event_obj, **kwargs):
        ...
        if event_obj:
            ...
            print(f"监听到新的属性异常：{ANOMALY_MAPPING[event_obj.element_type]}！尝试激活监听事件——仪玄闪能恢复！") if YIXUAN_REPORT else None
            self.listener_active()

    def listener_active(self):
        """监听事件激活，检测内置Cd，通过后为仪玄恢复闪能值。"""
        if not self.ready:
            print(f"仪玄在{self.last_active_tick}tick时已经通过该效果恢复过一次闪能值了，所以此时该效果的内置CD尚未就绪！") if YIXUAN_REPORT else None
            return
        else:
