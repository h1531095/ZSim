...
exist_buff_dict = {}
"""
sub_exist_buff_dict 是当前模拟中,所有可能被激活的buff列表.
...
def buff_exist_judge(charname_box, judge_list_set: list, weapon_dict):
    char_number = len(charname_box)
    weapon_judge_dict = weapon_dict
    ...
    for k in range(char_number):
        ...
        for i in allbuff_list:
            ...
            if (judged_buff.ft.bufffrom in judge_list_set[k]) or ((judged_buff.ft.bufffrom in total_judge_condition_list) and (judged_buff.ft.add_buff_to != 100)):
                if judged_buff.ft.is_weapon:
                    for char in charname_box:
                        if (judged_buff.ft.bufffrom == weapon_judge_dict[char][0]) and (judged_buff.ft.refinement == int(weapon_judge_dict[char][1])):
                            ...
                            sub_exist_buff_dict[i] = judged_buff
                            break
                else:
                    judged_buff.ft.exist = True
                    sub_exist_buff_dict[i] = judged_buff
            exist_buff_dict[charname_box[k]] = sub_exist_buff_dict
...
if __name__ == "__main__":      # 测试
    ...
    for buffs in exist_buff_dict['莱卡恩']:
        buff_now = exist_buff_dict['莱卡恩'][buffs]
