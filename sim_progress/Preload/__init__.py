...
#                 self.preload_data.preloaded_action.add(f)
...
#             selected_dict = {}
#             for CID, _node in self.preload_data.personal_current_node.items():
#                 selected_dict[CID] = self.spawn_lag_time(_node)
#             for selected_cid, lag_tick in selected_dict.items():
#                 if lag_tick == max(selected_dict.values()):
#                     current_node_CID = selected_cid
#                     lag_time = lag_tick
#                     current_node = self.preload_data.personal_current_node[current_node_CID]
#             # current_node_CID = int(skill_tag[:4])
