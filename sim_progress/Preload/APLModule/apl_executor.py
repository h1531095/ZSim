...
class APLExecutor:
    ...
    def execute(self, mode: int):
        ...
        for action in self.actions_list:
            all_conditions_list = action.get('conditions', None)
            ...
            result = self.distinguish_bool_logic(all_conditions_list, priority)
            if result:
                return self.perform_action(action['CID'], action["action"])
    ...
    def distinguish_bool_logic(self,cond: str, priority: int):
        if self.apl is None:
            raise ValueError("APLCondition is not initialized.")
        return self.apl.evaluate(cond, priority)
